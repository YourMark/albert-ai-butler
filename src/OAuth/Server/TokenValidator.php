...
use Exception;
use ExtendedAbilities\OAuth\Endpoints\Psr7Bridge;
	...
	public static function validate_request( WP_REST_Request $request ): WP_User|WP_Error {
		try {
			...
			if ( empty( $user_id ) ) {
				return new WP_Error(
					'oauth_invalid_token',
					__( 'Token does not contain a valid user identifier.', 'extended-abilities' ),
					[ 'status' => 401 ]
				);
			}
			...
			if ( ! $user ) {
				return new WP_Error(
					...
					__( 'User associated with token not found.', 'extended-abilities' ),
					[ 'status' => 401 ]
				);
			}
				...
				if ( ! in_array( $scope, $token_scopes, true ) ) {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'This action requires the "%s" scope.', 'extended-abilities' ),
							$scope
						),
						[ 'status' => 403 ]
					);
